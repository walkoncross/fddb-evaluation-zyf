...
#ifndef _WIN32
#include <unistd.h>
#else //added by zhaoyafei 20170824
#include "getopt.h"
#endif
#include <string>
#include <sstream>
...
//====added by zhaoyafei 2014-02-27
//#define SAVE_RESULT_IMAGES

#ifdef  SAVE_RESULT_IMAGES
#include <direct.h>
#define SAVE_PATH						"./rlt_images/"
#define SAVE_MATCHES_PATH	SAVE_PATH##"matched_faces/"
#define SAVE_FP_PATH				SAVE_PATH##"fp_faces/"
#define SAVE_MISSING_PATH		SAVE_PATH##"missing_faces/"
#endif
//====
...
//   string annotFile = "F:/scratch/Data/detectionResults/FDDB/ellipseList.txt";
  string baseDir = "E:/FDDB_UMass/originalPics/";
  string listFile = "E:/FDDB_UMass/FDDB-fold-01-10_2845.txt";
  string detFile = "./fd_result_fddb_format.txt";
  string annotFile = "E:/FDDB_UMass/FDDB-fold-01-10_2845-ellipseList.txt";
  ...
  //====added by zhaoyafei 2014-02-27
  bool saveRltImage = false;


#ifdef SAVE_RESULT_IMAGES
  rocFilePrefix.insert(0, string(SAVE_PATH));
  saveRltImage = true;

  mkdir(SAVE_PATH);
  mkdir(SAVE_MATCHES_PATH);
  mkdir(SAVE_FP_PATH);
  mkdir(SAVE_MISSING_PATH);
#else
  rocFilePrefix.insert(0, string("./"));
#endif
  //====
...
//#endif
  ...
  for(unsigned int ii = 0; ii < imNames->size(); ii++){
	...
	Results *r = new Results(imName, (std::numeric_limits<double>::max)()/*1.79769e+308*/, NULL, annot, det);
...
//====added by zhaoyafei 2014-02-27
		...
		//if(scoreThreshold>0)
		//{
		//	for(unsigned ai =0; ai<annot->length(); ai++)
		//	{
		//		Region *ra = annot->get(ai);
		//		printf("annot[%d]->matched=%d\n", ai, ra->matched==true?1:0);
		//	}

		//	for(unsigned di =0; di<det->length(); di++)
		//	{
		//		Region *rd = det->get(di);
		//		printf("det[%d]->matched=%d\n", di, rd->matched==true?1:0);
		//	}
		//}
#endif
//====added by zhaoyafei 2014-02-27
		...
		if(saveRltImage || showMatchPairs)
		{
...
//====added by zhaoyafei 2014-02-27
			...
			if(scoreThreshold>0)
			{
			  ...
			  strcpy(save_name, imFullName.c_str());
			  tmpPtr = save_name;

			  while(tmpPtr<save_name+strlen(save_name))
			  {
				  ...
				  tmpPtr++;
					...
					//====draw missing faces
					//if(!mps || (mps!=NULL && mps->size()<annot->length()))
					{
						for(unsigned ai =0; ai<annot->length(); ai++)
						{
							Region *ra= annot->get(ai);
							...
							{
								has_missing = true;
								mask = ( (EllipseR *)(ra) )->display(mask, CV_RGB(0,200,200), 3, NULL);;
							}
						}	
					}

					//====draw FP faces
					//if(!mps || (mps!=NULL && mps->size()<det->length() ))
					{
						for(unsigned di =0; di<det->length(); di++)
						{
							Region *rd = det->get(di);
							...
							{
								has_fp=true;
								mask = ( (RectangleR *)(rd) )->display(mask, CV_RGB(0,0,200), 3, NULL);;
							}
						}	
					}

					sprintf(save_name2, "%s%s", SAVE_MATCHES_PATH, save_name);
					cvSaveImage(save_name2, mask);

					if(has_fp==true)
					{
						sprintf(save_name2, "%s%s", SAVE_FP_PATH, save_name);
						cvSaveImage(save_name2, mask);
					}
					if(has_missing==true)
					{
						sprintf(save_name2, "%s%s", SAVE_MISSING_PATH, save_name);
						cvSaveImage(save_name2, mask);
					}
				}
#endif
